...
int main(void)
{
...

	{
		...
		unsigned char* ptr1 = (unsigned char*)glMapBuffer(GL_UNIFORM_BUFFER, GL_WRITE_ONLY);//glMapBuffer returns void pointer
		...
		if (ptr1 != NULL) {
			...
			for (int i = 0; i < 4; ++i) {
				...
				GLuint offset2 = 64+ 16 * i;//rotation
				GLuint offset3 = 128 + 16 * i;//projection
				for (int j = 0; j < 4; ++j) {
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			const GLfloat color[] = { (float)sin(r) * 0.5f + 0.5f, (float)cos(r) * 0.5f + 0.5f, 0.0f, 1.0f };
